    ...
    const placeOrder = async () => {
        ...
        try {
            ...
            if(paymentOption === "COD"){
                const { data } = await axios.post('/api/order/cod',{items: cartArray.map(item => ({product:item._id, quantity: item.quantity})),
                ...
                //place Order with Stripe
                    const { data } = await axios.post('/api/order/stripe',{items: cartArray.map(item => ({product:item._id, quantity: item.quantity})),
                address: selectedAddress._id})
                if (data.success) {
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        ...
                                        <select onChange={e => updateCartItem(product._id, Number(e.target.value))} value={cartItems[product._id]} className='outline-none'>
                                            {Array(cartItems[product._id] > 100 ? cartItems[product._id] : 100).fill('').map((_, index) => (
